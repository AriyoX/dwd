...
Remote mutation is intentionally blocked until the project identity is known. Use this sequence:
...
The migration is [supabase/migrations/20260731195723_initial_dwd_schema.sql](supabase/migrations/20260731195723_initial_dwd_schema.sql). Its timestamped name was created by `supabase migration new`, not hand-invented.
...
See [docs/deployment.md](docs/deployment.md) for CLI commands and the remaining account setup.
...
- The dedicated Vercel project is created and linked. Supabase provisioning and publication remain pending the user's chosen organization; remote database advisors and the four-session Auth/Realtime browser checklist remain deployment work. See [deployment status](docs/deployment.md).
...
- Uganda emergency numbers `112` and `999` are configuration values marked as verified on 2026-07-31; an official Ugandan source must be checked again immediately before production release.
